CALL {
  ...
  OPTIONAL MATCH (a:ActoAdministrativo)
  WHERE entity_key <> '' AND toUpper(coalesce(a.search_text, '')) CONTAINS toUpper(entity_key)
  OPTIONAL MATCH (g:GacetaTerritorial)
  WHERE entity_key <> '' AND toUpper(coalesce(g.search_text, '')) CONTAINS toUpper(entity_key)
  WITH collect(DISTINCT coalesce(award.summary_id, award.contract_id)) AS award_refs,
       collect(DISTINCT coalesce(a.act_id, a.number)) AS act_refs,
       collect(DISTINCT coalesce(g.gaceta_id, g.number)) AS gazette_refs
  WITH [x IN award_refs + act_refs + gazette_refs WHERE x IS NOT NULL AND x <> ''][0..toInteger($pattern_max_evidence_refs)] AS evidence_refs,
       size(act_refs) + size(gazette_refs) AS doc_count
  WHERE doc_count > 0
  RETURN coalesce(act_refs[0], gazette_refs[0], entity_key) AS scope_key,
         toFloat(doc_count + size(award_refs)) AS risk_signal,
         evidence_refs,
         size(evidence_refs) AS evidence_count,
         'EXACT_COMPANY_NIT' AS identity_match_type,
         'exact' AS identity_quality,
         'SourceDocument:' + coalesce(act_refs[0], gazette_refs[0], entity_key) AS node_ref
  UNION
  WITH $entity_id AS entity_id, $entity_key AS entity_key, $entity_label AS entity_label
