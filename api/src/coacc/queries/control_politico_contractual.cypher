CALL {
  ...
  OPTIONAL MATCH (r:InquiryRequirement)
  WHERE entity_key <> '' AND toUpper(coalesce(r.search_text, '')) CONTAINS toUpper(entity_key)
  OPTIONAL MATCH (s:InquirySession)
  WHERE entity_key <> '' AND toUpper(coalesce(s.search_text, '')) CONTAINS toUpper(entity_key)
  WITH collect(DISTINCT coalesce(award.summary_id, award.contract_id)) AS award_refs,
       collect(DISTINCT coalesce(r.requirement_id, r.code)) AS requirement_refs,
       collect(DISTINCT coalesce(s.session_id, s.session_no)) AS session_refs
  WITH [x IN award_refs + requirement_refs + session_refs WHERE x IS NOT NULL AND x <> ''][0..toInteger($pattern_max_evidence_refs)] AS evidence_refs,
       size(requirement_refs) + size(session_refs) AS oversight_count
  WHERE oversight_count > 0
  RETURN coalesce(requirement_refs[0], session_refs[0], entity_key) AS scope_key,
         toFloat(oversight_count + size(award_refs)) AS risk_signal,
         evidence_refs,
         size(evidence_refs) AS evidence_count,
         'EXACT_COMPANY_NIT' AS identity_match_type,
         'exact' AS identity_quality,
         'Inquiry:' + coalesce(requirement_refs[0], session_refs[0], entity_key) AS node_ref
  UNION
  WITH $entity_id AS entity_id, $entity_key AS entity_key, $entity_label AS entity_label
