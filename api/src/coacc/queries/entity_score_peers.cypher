...
WITH peer, conn_count, COALESCE(sum(c.value), 0) AS contract_node_vol
OPTIONAL MATCH ()-[award:CONTRATOU]->(peer)
WITH peer, conn_count, contract_node_vol + COALESCE(sum(award.total_value), 0) AS contract_vol
