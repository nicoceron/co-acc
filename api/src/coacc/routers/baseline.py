...
from coacc.models.baseline import BaselineResponse
from coacc.services.baseline_service import BASELINE_QUERIES, run_all_baselines, run_baseline
from coacc.services.lakehouse_baseline_service import lake_all_baselines, lake_baseline
...
async def get_baseline_for_entity(
    ...
    if dimension:
        ...
        results = lake_baseline(entity_id, dimension=dimension)
        if not results and session is not None:
            results = await run_baseline(session, dimension, entity_id)
    else:
        results = lake_all_baselines(entity_id)
        if not results and session is not None:
