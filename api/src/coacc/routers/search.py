...
def _extract_name(node: Any, labels: list[str]) -> str:
    ...
    if entity_type == "publicOffice":
        return str(props.get("name", props.get("role_name", props.get("org", ""))))
    if entity_type == "bid":
        return str(props.get("name", props.get("reference", props.get("bid_id", ""))))
    if entity_type == "declaredAsset":
        return str(props.get("name", props.get("form_id", props.get("asset_id", ""))))
    if entity_type == "finance":
        return str(props.get("name", props.get("type", "")))
...
async def search_entities(
    ...
    for record in records:
        ...
        # Only expose public document identifiers, not internal element IDs
