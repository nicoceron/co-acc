...
from coacc.models.entity import ExposureFactor, ExposureResponse, SourceAttribution
...
COMMUNITY_COMPANY_PATTERN_QUERIES: dict[str, str] = {
    "sanctioned_supplier_record": "public_pattern_sanctioned_supplier_record",
...
COMMUNITY_PATTERN_QUERIES = {
    **COMMUNITY_COMPANY_PATTERN_QUERIES,
    **COMMUNITY_PERSON_PATTERN_QUERIES,
}
...
def _load_pattern_queries() -> Mapping[str, str]:
    module = import_module("coacc.services.pattern_service")
...
def _load_compute_exposure() -> _ComputeExposure:
    module = import_module("coacc.services.score_service")
    module_any = cast("Any", module)
...
def _community_person_pattern_params(
    person_id: str,
    person_document_id: str,
) -> dict[str, str | int | float]:
    return {
        "person_id": person_id,
        "person_document_id": person_document_id,
        "pattern_max_evidence_refs": settings.pattern_max_evidence_refs,
        "pattern_min_contract_value": settings.pattern_min_contract_value,
        "pattern_min_contract_count": settings.pattern_min_contract_count,
    }
...
class CommunityIntelligenceProvider:
    ...
    async def run_pattern(
        ...
        person = None if company is not None else await self._resolve_person(session, entity_id)

        pattern_ids: tuple[str, ...]
        if pattern_id == "__all__":
            pattern_ids = COMMUNITY_ANALYSIS_PATTERN_IDS
        ...
        if company is not None:
            ...
            allowed_pattern_ids = set(COMMUNITY_COMPANY_PATTERN_IDS)
            entity_ids = [company_id]
        elif person is not None:
            person_id, person_document_id = person
            params = _community_person_pattern_params(person_id, person_document_id)
            allowed_pattern_ids = set(COMMUNITY_PERSON_PATTERN_IDS)
        ...
        for pid in pattern_ids:
            if pid not in allowed_pattern_ids:
                continue
            try:
    ...
    async def _resolve_public_entity_record(
        self,
        session: AsyncSession,
        entity_id: str,
    ) -> Any:
        ...
        if record is not None:
            return record
        return await execute_query_single(
            session,
            "entity_by_id",
            {"id": entity_id},
        )
        ...
        if by_element is not None and "Company" in by_element["entity_labels"]:
            ...
            for key in ("document_id", "nit"):
                ...
                if _DOCUMENT_PATTERN.match(digits):
                    return entity_id, digits, digits

        identifier = re.sub(r"[.\-/]", "", entity_id)
        if not _DOCUMENT_PATTERN.match(identifier):
            return None
...
def _full_modules_available() -> bool:
    return (
        find_spec("coacc.services.pattern_service") is not None
        and find_spec("coacc.services.score_service") is not None
    )
