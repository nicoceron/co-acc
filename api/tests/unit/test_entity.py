...
async def test_entity_lookup_rejects_short_number(client: AsyncClient) -> None:
    response = await client.get("/api/v1/entity/1234")
    assert response.status_code == 400
...
# ── entity_by_id: all supported ID fields are present in WHERE clause ──────────
...
# ── Investigation coalesce chains: all tracked queries include all supported ID fields ──
...
def test_investigation_coalesce_has_all_id_fields(query_name: str) -> None:
    """Every investigation coalesce chain must include all supported entity ID fields."""
...
def test_investigation_where_has_all_id_fields(query_name: str) -> None:
    """Investigation add/remove WHERE clauses must look up all supported entity ID fields."""
    cypher = _load_cypher(query_name)
    for field in ALL_ID_FIELDS:
        assert field in cypher, (
