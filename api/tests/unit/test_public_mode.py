...
async def test_person_lookup_disabled_in_public_mode(
    ...
    with patch(
        "coacc.routers.entity.execute_query_single",
        new_callable=AsyncMock,
        return_value={
            "e": {"name": "Persona Teste", "cedula": "12345678901"},
        ...
        response = await client.get("/api/v1/entity/12345678901")
    assert response.status_code == 403
    assert "Person-level entities disabled" in response.json()["detail"]
...
async def test_search_hides_person_nodes_in_public_mode(
    ...
    mocked_records = [
        {
            "node": {"name": "Persona Teste", "cedula": "12345678900"},
            "node_labels": ["Person"],
            "node_id": "p1",
            "score": 3.1,
    ...
    with patch(
        "coacc.routers.search.execute_query",
        new_callable=AsyncMock,
...
async def test_public_patterns_company_endpoint(client: AsyncClient) -> None:
    with patch("coacc.routers.public.settings.patterns_enabled", False):
...
async def test_public_patterns_company_endpoint_when_enabled(client: AsyncClient) -> None:
    with (
        patch("coacc.routers.public.settings.patterns_enabled", True),
        patch(
            "coacc.routers.public.execute_query_single",
            new_callable=AsyncMock,
            return_value={
        ...
        patch(
            "coacc.routers.public._PUBLIC_PROVIDER.run_pattern",
            new_callable=AsyncMock,
            return_value=[
                PatternResult(
...
async def test_public_graph_company_filters_person_nodes(client: AsyncClient) -> None:
    with (
        ...
        patch(
            "coacc.routers.public.execute_query",
            new_callable=AsyncMock,
            return_value=[
                {
                    "nodes": [
...
async def test_stats_hides_person_count_in_public_mode(
    ...
    import coacc.routers.meta as meta_mod
    ...
    with patch(
        ...
        "coacc.routers.meta.load_source_registry",
        return_value=[],
    ), patch(
        "coacc.routers.meta.source_registry_summary",
        return_value={
...
async def test_timeline_sanitizes_properties_in_public_mode(
    ...
    mock_records = [
        {
            "lbls": ["Contract"],
            "props": {"type": "licitacao", "cedula": "12345678900", "value": 50000.0},
            "event_date": "2024-01-15",
            "id": "evt-1",
        },
    ]
    with patch(
        "coacc.routers.entity.execute_query",
        new_callable=AsyncMock,
    ...
    assert "cedula" not in event_props
