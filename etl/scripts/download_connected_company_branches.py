#!/usr/bin/env python3
"""Download nb3d-v3n7 establishment rows connected to the live graph identity universe."""
...
DATASET_ID = "nb3d-v3n7"
OUTPUT_RELPATH = "company_branches_nb3d/company_branches_nb3d.csv"
KEY_FIELDS = ("codigo_camara", "matricula", "nit_propietario", "numero_identificacion")
...
def _materialized_results_path() -> Path:
    return _repo_root() / "frontend" / "public" / "data" / "materialized-results.json"
...
def _connected_ids() -> tuple[str, ...]:
    materialized_path = _materialized_results_path()
    if materialized_path.exists():
        payload = json.loads(materialized_path.read_text(encoding="utf-8"))
        companies = payload.get("watchlists", {}).get("companies", [])
        seeded: list[str] = []
        seen_seeded: set[str] = set()
        for row in companies:
            normalized = _normalize_identifier(row.get("document_id"))
            if not normalized or normalized in seen_seeded:
                continue
            seen_seeded.add(normalized)
    ...
    query = """
        MATCH (c:Company)
        WHERE EXISTS { MATCH ()-[r:CONTRATOU]->(c) }
           OR EXISTS { MATCH (:Person)-[:OFFICER_OF]->(c) }
           OR EXISTS { MATCH (c)-[:SANCIONADA]->() }
           OR EXISTS { MATCH (c)-[:CELEBRO_CONVENIO_INTERADMIN]->() }
        RETURN DISTINCT coalesce(c.nit, c.document_id) AS value
    """
    combined: list[str] = []
    with driver.session(database=os.getenv("NEO4J_DATABASE", "neo4j")) as session:
        for record in session.run(query):
            value = clean_text(record["value"])
            if value:
...
def _download_connected_dataset(
    *,
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with TemporaryDirectory(prefix="company-branches-", dir=output_path.parent) as tmp_dir:
...
def main(data_dir: str, timeout: int, batch_size: int, max_clause_chars: int) -> None:
    identifiers = _connected_ids()
    if not identifiers:
        raise click.ClickException("No connected identifiers found in the live graph.")
