...
class DnpProjectBeneficiaryLocationsPipeline(Pipeline):
    """Load DNP beneficiary geography summaries keyed by BPIN."""

    name = "dnp_project_beneficiary_locations"
    source_id = "dnp_project_beneficiary_locations"
    ...
    def extract(self) -> None:
        csv_path = (
            ...
            / "dnp_project_beneficiary_locations.csv"
        )
    ...
    def transform(self) -> None:
        ...
        for row in self._raw.to_dict(orient="records"):
            ...
            beneficiary_total = parse_integer(row.get("totalbeneficiario"))
            ...
            current["beneficiary_locations"] = merge_limited_unique(
                list(current.get("beneficiary_locations", [])),
                location_name,
                limit=12,
            )
            current["beneficiary_location_count"] = len(current["beneficiary_locations"])
            current["beneficiary_total_reported"] = max(
                int(current.get("beneficiary_total_reported") or 0),
                int(beneficiary_total or 0),
            )
