...
import { formatPropertyLabel, formatSourceName } from "@/lib/display";
...
interface InsightsPanelProps {
  ...
  patternsStatus: "idle" | "ready" | "unavailable" | "error";
  ...
  return (
    <div className={styles.insightsScroll}>
      ...
      <section className={styles.section}>
        ...
        {patternsStatus === "unavailable" ? (
          <p className={styles.statusCard}>{t("analysis.patternsUnavailable")}</p>
        ) : patternsStatus === "error" ? (
          <p className={styles.statusCard}>{t("patterns.loadError")}</p>
      ...
      <dl className={styles.propList}>
        {entries.map(([key, val]) => (
          <div key={key} className={styles.propRow}>
            <dt className={styles.propKey}>{formatPropertyLabel(key)}</dt>
