...
function ControlsSidebarInner({
  ...
  const visibleEntityTypes = Object.entries(typeCounts)
    .sort(([, left], [, right]) => right - left)
    .map(([type]) => type as DataEntityType);
  const visibleRelationshipTypes = Object.entries(relTypeCounts)
    .sort(([, left], [, right]) => right - left)
    .map(([type]) => type);
              ...
              {visibleRelationshipTypes.map((type) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span className={styles.toggleLabel}>
                      {t(`relationship.${type}`, humanizeCode(type))}
                    </span>
                    <span className={styles.count}>{count}</span>
                  </button>
                );
