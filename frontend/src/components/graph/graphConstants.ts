...
export const ICON_MAP: Record<string, string> = {
  ...
  amendment: "FileEdit",
  publicOffice: "BriefcaseBusiness",
  health: "Heart",
  finance: "DollarSign",
  embargo: "Ban",
...
function createIconSvg(type: string, color: string, size: number): string {
  ...
  const shapes: Record<string, string> = {
    ...
    publicOffice: `<rect x="${s * 0.2}" y="${s * 0.28}" width="${s * 0.6}" height="${s * 0.5}" rx="2" fill="${color}"/><rect x="${s * 0.35}" y="${s * 0.14}" width="${s * 0.3}" height="${s * 0.18}" rx="1" fill="${color}"/>`,
