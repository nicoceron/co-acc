...
describe("SearchBar", () => {
  it("renders input, select, and button", () => {
    ...
    expect(screen.getByPlaceholderText(/NIT/i)).toBeInTheDocument();
  ...
  it("calls onSearch with query and type on submit", async () => {
    ...
    await user.type(screen.getByPlaceholderText(/NIT/i), "João Silva");
  ...
  it("allows selecting a type filter", async () => {
    ...
    await user.type(screen.getByPlaceholderText(/NIT/i), "Acme");
    await user.click(screen.getByRole("button"));
