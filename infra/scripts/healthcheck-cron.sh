...
CRON_ENTRY="*/5 * * * * ${HEALTH_SCRIPT} >> /var/log/coacc-health.log 2>&1"
...
if [ -n "$ALERT_WEBHOOK_URL" ]; then
    curl -sf -X POST "$ALERT_WEBHOOK_URL" \
        ...
        -d "{\"text\":\"CO-ACC health check failed at $(date '+%Y-%m-%d %H:%M:%S') — ${HEALTH_URL}\"}" \
        > /dev/null 2>&1 || true
fi
SCRIPT
...
echo "Logs: /var/log/coacc-health.log"
