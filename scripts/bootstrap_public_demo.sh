...
for _ in $(seq 1 60); do
  if docker exec coacc-neo4j cypher-shell -u neo4j -p "$NEO4J_PASSWORD" "RETURN 1" >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
...
if [[ "$PROFILE" == "full" ]]; then
  ...
  for source in "${pipeline_array[@]}"; do
    ...
    (cd etl && uv run coacc-etl run --source "$source" --neo4j-uri "$NEO4J_URI" --neo4j-password "$NEO4J_PASSWORD" --neo4j-database "$NEO4J_DATABASE" --data-dir ../data)
  done
fi

seed_nodes="$(docker exec coacc-neo4j cypher-shell -u neo4j -p "$NEO4J_PASSWORD" "MATCH (n) RETURN count(n) AS c" --format plain 2>/dev/null | tail -n 1 | tr -d '[:space:]')"
